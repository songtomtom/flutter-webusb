...
class UsbDevice extends Delegate<Object> {
  ...
  Future<void> controlTransferOut(ControlTransferOutSetup setup) {
    var promise = callMethod('controlTransferOut', [setup]);
    return promiseToFuture(promise);
  }
}
...
@JS()
@anonymous
class ControlTransferOutSetup {
  external factory ControlTransferOutSetup({
    String requestType,
    String recipient,
    int request,
    int value,
...
class UsbAlternateInterface extends Delegate<Object> {
  ...
  List<UsbAlternateEndpoints> get endpoints {
    var property = getProperty('endpoints');
    if (property == null) return [];
    return (property as List).map((e) => UsbAlternateEndpoints._(e)).toList();
  }
}

class UsbAlternateEndpoints extends Delegate<Object> {
  UsbAlternateEndpoints._(Object delegate) : super(delegate);
